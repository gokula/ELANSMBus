...
#include "VoodooCSGestureHIPointingWrapperAS.hpp"
#include "VoodooI2CCSGestureEngineAS.hpp"
#include <libkern/version.h>


OSDefineMetaClassAndStructors(VoodooCSGestureHIPointingWrapperAS, IOHIPointing);

UInt32 VoodooCSGestureHIPointingWrapperAS::deviceType()
{
    return NX_EVS_DEVICE_TYPE_MOUSE;
}

UInt32 VoodooCSGestureHIPointingWrapperAS::interfaceID()
{
    return NX_EVS_DEVICE_INTERFACE_BUS_ACE;
}

IOItemCount VoodooCSGestureHIPointingWrapperAS::buttonCount(){
    return 2;
};

IOFixed VoodooCSGestureHIPointingWrapperAS::resolution(){
    return (300) << 16;
};

bool VoodooCSGestureHIPointingWrapperAS::init(){
    if (!super::init())
        return false;
...
bool VoodooCSGestureHIPointingWrapperAS::start(IOService *provider){
...
void VoodooCSGestureHIPointingWrapperAS::stop(IOService *provider){
    super::stop(provider);
}

void VoodooCSGestureHIPointingWrapperAS::updateRelativeMouse(int dx, int dy, int buttons){
    // 0x1 = left button
    // 0x2 = right button
    // 0x4 = middle button
...
void VoodooCSGestureHIPointingWrapperAS::updateAbsoluteMouse(SInt16 x, SInt16 y, int buttons){
    // 0x1 = left button
    // 0x2 = right button
    // 0x4 = middle button
...
void VoodooCSGestureHIPointingWrapperAS::updateScroll(short dy, short dx, short dz){
    uint64_t now_abs;
    clock_get_uptime(&now_abs);
    if (!horizontalScroll)
        dx = 0;
...
IOReturn VoodooCSGestureHIPointingWrapperAS::setParamProperties(OSDictionary *dict)
{
