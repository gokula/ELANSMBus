//
//  VoodooI2CCSGestureEngineAS.cpp
//  VoodooI2C
//
...
#include "../../Dependencies/Headers/libkern/libkern/OSBaseA.h"
...
OSDefineMetaClassAndStructors(VoodooI2CCSGestureEngineAS, VoodooI2CMultitouchEngineAS);
...
int VoodooI2CCSGestureEngineAS::distancesq(int delta_x, int delta_y){
    return (delta_x * delta_x) + (delta_y*delta_y);
}

MultitouchReturn VoodooI2CCSGestureEngineAS::handleInterruptReport(VoodooI2CMultitouchEvent event, AbsoluteTime timestamp) {
    int i;
    ...
    for (i=0; i < event.transducers->getCount(); i++) {
        VoodooI2CDigitiserTransducerAS* transducer = OSDynamicCast(VoodooI2CDigitiserTransducerAS, event.transducers->getObject(i));
        if (!transducer)
            continue;
...
void VoodooI2CCSGestureEngineAS::timedProcessGesture() {
    ProcessGesture(&softc);
    
    this->timer_event_source->setTimeoutMS(5);
}

bool VoodooI2CCSGestureEngineAS::ProcessMove(csgesture_softc *sc, int abovethreshold, int iToUse[4]) {
...
bool VoodooI2CCSGestureEngineAS::ProcessScroll(csgesture_softc *sc, int abovethreshold, int iToUse[4]) {
    int frequmult = 10 / sc->frequency;
...
bool VoodooI2CCSGestureEngineAS::ProcessThreeFingerSwipe(csgesture_softc *sc, int abovethreshold, int iToUse[4]) {
    if (abovethreshold == 3) {
        if (_scrollHandler){
            _scrollHandler->softc = sc;
            _scrollHandler->stopScroll();
...
bool VoodooI2CCSGestureEngineAS::ProcessFourFingerSwipe(csgesture_softc *sc, int abovethreshold, int iToUse[4]) {
    if (abovethreshold == 4) {
        _scrollHandler->softc = sc;
        _scrollHandler->stopScroll();
...
void VoodooI2CCSGestureEngineAS::TapToClickOrDrag(csgesture_softc *sc, int button) {
...
void VoodooI2CCSGestureEngineAS::ClearTapDrag(csgesture_softc *sc, int i) {
    int frequmult = 10 / sc->frequency;
...
void VoodooI2CCSGestureEngineAS::ProcessGesture(csgesture_softc *sc) {
    int frequmult = 10 / sc->frequency;
...
void VoodooI2CCSGestureEngineAS::prepareToSleep(){
    if (_scrollHandler)
...
void VoodooI2CCSGestureEngineAS::wakeFromSleep(){
    if (_scrollHandler)
        _scrollHandler->wakeFromSleep();
    
    if (!this->timer_event_source){
        this->timer_event_source = IOTimerEventSource::timerEventSource(this, OSMemberFunctionCast(IOTimerEventSource::Action, this, &VoodooI2CCSGestureEngineAS::timedProcessGesture));
        this->work_loop->addEventSource(this->timer_event_source);
        this->timer_event_source->setTimeoutMS(10);
    }
}

bool VoodooI2CCSGestureEngineAS::start(IOService *service) {
    ...
    _wrapper = new VoodooCSGestureHIDWrapperAS;
    if (_wrapper->init()) {
    ...
    _pointingWrapper = new VoodooCSGestureHIPointingWrapperAS;
    ...
    _scrollHandler = new CSGestureScrollAS;
...
void VoodooI2CCSGestureEngineAS::stop(IOService* provider) {
...
void VoodooI2CCSGestureEngineAS::update_relative_mouse(char button, char x, char y, char wheelPosition, char wheelHPosition){
    if (_pointingWrapper)
        _pointingWrapper->updateRelativeMouse(x, y, button);
}

void VoodooI2CCSGestureEngineAS::update_absolute_mouse(char button, SInt16 x, SInt16 y){
    if (_pointingWrapper)
        _pointingWrapper->updateAbsoluteMouse(x, y, button);
}

void VoodooI2CCSGestureEngineAS::update_keyboard(uint8_t shiftKeys, uint8_t keyCodes[KBD_KEY_CODES]) {
...
int VoodooI2CCSGestureEngineAS::reportDescriptorLength(){
    return sizeof(csgesturedesc);
}

void VoodooI2CCSGestureEngineAS::write_report_to_buffer(IOMemoryDescriptor *buffer){
...
void VoodooI2CCSGestureEngineAS::write_report_descriptor_to_buffer(IOMemoryDescriptor *buffer){
