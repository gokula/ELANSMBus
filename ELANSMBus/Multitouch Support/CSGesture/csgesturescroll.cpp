...
OSDefineMetaClassAndStructors(CSGestureScrollAS, IOService);
...
void CSGestureScrollAS::disableScrollDelayed(){
...
void CSGestureScrollAS::disableScrollingDelayLaunch(){
    ...
    _disableScrollDelayTimer = IOTimerEventSource::timerEventSource(this, OSMemberFunctionCast(IOTimerEventSource::Action, this, &CSGestureScrollAS::disableScrollDelayed));
    _workLoop->addEventSource(_disableScrollDelayTimer);
    _disableScrollDelayTimer->setTimeoutMS(300);
}

bool CSGestureScrollAS::isScrolling(){
    if (isTouchActive)
        return true;
...
void CSGestureScrollAS::stopScroll(){
...
void CSGestureScrollAS::scrollTimer(){
...
void CSGestureScrollAS::ProcessScroll(int x1, int y1, int x2, int y2) {
...
void CSGestureScrollAS::prepareToSleep(){
    if (_scrollTimer){
...
void CSGestureScrollAS::wakeFromSleep(){
    if (_scrollTimer){
        _scrollTimer->cancelTimeout();
        _scrollTimer->release();
        _scrollTimer = NULL;
    ...
    _scrollTimer = IOTimerEventSource::timerEventSource(this, OSMemberFunctionCast(IOTimerEventSource::Action, this, &CSGestureScrollAS::scrollTimer));
    _workLoop->addEventSource(_scrollTimer);
    _scrollTimer->setTimeoutMS(10);
}

bool CSGestureScrollAS::start(IOService *provider){
