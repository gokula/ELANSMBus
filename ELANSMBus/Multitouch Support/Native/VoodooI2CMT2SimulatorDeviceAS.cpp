...
OSDefineMetaClassAndStructors(VoodooI2CMT2SimulatorDeviceAS, IOHIDDevice);
...
void VoodooI2CMT2SimulatorDeviceAS::constructReport(VoodooI2CMultitouchEvent multitouch_event, AbsoluteTime timestamp) {
    if (!ready_for_reports)
        return;

    command_gate->runAction(OSMemberFunctionCast(IOCommandGate::Action, this, &VoodooI2CMT2SimulatorDeviceAS::constructReportGated), &multitouch_event, &timestamp);
}

void VoodooI2CMT2SimulatorDeviceAS::constructReportGated(VoodooI2CMultitouchEvent& multitouch_event, AbsoluteTime& timestamp) {
    if (!ready_for_reports)
        return;
    ...
    VoodooI2CDigitiserTransducerAS* transducer = OSDynamicCast(VoodooI2CDigitiserTransducerAS, multitouch_event.transducers->getObject(0));
    ...
    for (int i = 0; i < multitouch_event.contact_count + 1; i++) {
        VoodooI2CDigitiserTransducerAS* transducer = OSDynamicCast(VoodooI2CDigitiserTransducerAS, multitouch_event.transducers->getObject(i));
...
bool VoodooI2CMT2SimulatorDeviceAS::start(IOService* provider) {
    if (!super::start(provider))
        return false;
    
    clock_get_uptime(&start_timestamp);
    
    engine = OSDynamicCast(VoodooI2CNativeEngineAS, provider);
...
void VoodooI2CMT2SimulatorDeviceAS::stop(IOService* provider) {
    releaseResources();
...
IOReturn VoodooI2CMT2SimulatorDeviceAS::setPowerState(unsigned long whichState, IOService* whatDevice) {
    if (whatDevice != this)
...
void VoodooI2CMT2SimulatorDeviceAS::releaseResources() {
    if (command_gate) {
...
IOReturn VoodooI2CMT2SimulatorDeviceAS::setReport(IOMemoryDescriptor* report, IOHIDReportType reportType, IOOptionBits options) {
...
IOReturn VoodooI2CMT2SimulatorDeviceAS::getReport(IOMemoryDescriptor* report, IOHIDReportType reportType, IOOptionBits options) {
    UInt32 report_id = options & 0xFF;
...
IOReturn VoodooI2CMT2SimulatorDeviceAS::newReportDescriptor(IOMemoryDescriptor** descriptor) const {
...
OSString* VoodooI2CMT2SimulatorDeviceAS::newManufacturerString() const {
    return OSString::withCString("Apple Inc.");
}

OSNumber* VoodooI2CMT2SimulatorDeviceAS::newPrimaryUsageNumber() const {
    return OSNumber::withNumber(kHIDUsage_GD_Mouse, 32);
}

OSNumber* VoodooI2CMT2SimulatorDeviceAS::newPrimaryUsagePageNumber() const {
    return OSNumber::withNumber(kHIDPage_GenericDesktop, 32);
}

OSNumber* VoodooI2CMT2SimulatorDeviceAS::newProductIDNumber() const {
    return OSNumber::withNumber(0x272, 32);
}

OSString* VoodooI2CMT2SimulatorDeviceAS::newProductString() const {
    return OSString::withCString("Magic Trackpad 2");
}

OSString* VoodooI2CMT2SimulatorDeviceAS::newSerialNumberString() const {
    return OSString::withCString("VoodooI2C Magic Trackpad 2 Simulator");
}

OSString* VoodooI2CMT2SimulatorDeviceAS::newTransportString() const {
    return OSString::withCString("I2C");
}

OSNumber* VoodooI2CMT2SimulatorDeviceAS::newVendorIDNumber() const {
    return OSNumber::withNumber(0x5ac, 16);
}

OSNumber* VoodooI2CMT2SimulatorDeviceAS::newLocationIDNumber() const {
    return OSNumber::withNumber(0x14400000, 32);
}

OSNumber* VoodooI2CMT2SimulatorDeviceAS::newVersionNumber() const {
